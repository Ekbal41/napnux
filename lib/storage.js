...
class Storage {
   ...
   * @returns {Promise<string>} A promise that resolves to the stored file's path.
   */
   ...
   * @returns {Promise<void>} A promise that resolves when the file is removed or if it doesn't exist.
   */
  remove(filename) {
    const filePath = path.join(this.storageDir, filename);
