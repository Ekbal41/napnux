const njucks = require("./middleware/njucks");
...
module.exports = (napnux) => {
    let userDirectory = path.dirname(require.main.filename);

    // default static and views setup
    // for projet
    let { staticDir: pstaticDir, viewsDir: pviewsDir } = napnux.settings;
    if (pstaticDir) {
        let pstaticPath = path.join(userDirectory, pstaticDir);
        defaultstatic(napnux, pstaticPath);
    }
    if (pviewsDir) {
        let pviewsPath = path.join(userDirectory, pviewsDir);
    ...
    appKeys.forEach((appKey) => {
        let { staticDir, viewsDir, appsDir } = napnux.apps[appKey].settings;
        if (staticDir) {
            let appStaticPath = path.join(userDirectory, appsDir, appKey, staticDir)
            defaultstatic(napnux.apps[appKey], appStaticPath)
        }
        if (viewsDir) {
            let appViewsPath = path.join(userDirectory, appsDir, appKey, viewsDir)
...
const defaultviews = (napnux, root, opts = {}) => {
    napnux.use(njucks({ views: root, ...opts }));
};
